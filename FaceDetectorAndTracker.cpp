...
{
    m_camera = make_unique<cv::VideoCapture>(cameraIndex);
    ...
    m_faceCascade = make_unique<cv::CascadeClassifier>(cascadeFilePath);
    ...
    image = cv::imread("data/images/1.bmp", 1);
}
...
void FaceDetectorAndTracker::operator>>(cv::Mat &frame)
{
    ...
    *m_camera >> frame;
    ...
    if(image.data && use_image){
        image.copyTo(frame(cv::Rect(0, 0, image.cols, image.rows)));
    }
    /////////////
